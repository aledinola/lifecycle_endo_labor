...
The current Fortran source solves the baseline model using value function iteration, computes the household distribution over state space, aggregates cohort outcomes, and writes the table `output.txt` when run. It also writes `runtime_report.txt` with total runtime and separate timings for `solve_household`, `get_distribution`, and `aggregation`.
